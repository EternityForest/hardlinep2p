...
class Service():
    def __init__(self,cert, destination,port):
        self.certfile = cert
    ...
    def handleConnection(self,sock):
        ...
        def f():
            ...
            while(1):
                r,w,x = select.select([sock,conn],[],[sock,conn],1)
                try:
...
def server_thread(sock):
    ...
    def f():
        ...
        while(1):
            r,w,x = select.select([sock,conn],[conn] if rb else [],[sock,conn],1)
            try:
...
s= Service("foo.cert","localhost",80)

#This is the server context we use for localhost coms

bindsocket = socket.socket()
bindsocket.bind(('localhost', 7008))
bindsocket.listen()
...
while(1):
    ...
    r,w,x = select.select([bindsocket,p2p_bindsocket],[],[bindsocket,p2p_bindsocket],1)
    try:
        if x:
